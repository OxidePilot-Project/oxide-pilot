...
async fn analyze_with_openai(snapshot: &Value) -> Result<ModelReport, String> {
  ...
  match openai_client::chat_completion(&model_name, messages, Some(0.1), None).await {
    Ok(text) => match serde_json::from_str::<ModelReport>(&text) {
      Ok(mut mr) => { mr.provider = "openai".to_string(); Ok(mr) },
      Err(e) => {
        ...
        Ok(ModelReport{
          ...
          findings: vec![],
          indicators: vec![],
          recommendations: vec!["Manual review recommended; model returned unstructured output".to_string()],
          citations: vec![],
        })
      }
    },
...
pub struct ThreatFinding {
  ...
  #[serde(default)] pub indicators: Vec<String>,
}
...
pub struct ThreatReport {
  ...
  #[serde(default)] pub findings: Vec<ThreatFinding>,
  #[serde(default)] pub indicators: Vec<Indicator>,
  #[serde(default)] pub recommendations: Vec<String>,
  #[serde(default)] pub citations: Vec<Citation>,
  #[serde(default)] pub disagreement_alerts: Vec<String>,
  pub evidence: Value,
...
fn aggregate(mut reports: Vec<ModelReport>, evidence: Value) -> ThreatReport {
  ...
  let mode = if providers.len() >= 2 { "dual" } else { "single" }.to_string();
  ...
  let confidence = (reports.iter().map(|r| normalize_conf(r.confidence)).sum::<f32>() / reports.len() as f32)
    .clamp(0.0, 1.0);
  ...
  if providers.len() >= 2 {
    ...
    if kinds.len() >= 4 { disagreement_alerts.push("High diversity of finding kinds; review manually".to_string()); }
  }
  ...
  match auth.send_message(&prompt, Some("gemini-1.5-pro")).await {
    Ok(text) => {
      ...
      match serde_json::from_str::<ModelReport>(&text) {
        Ok(mut mr) => { mr.provider = "gemini".to_string(); Ok(mr) },
        Err(e) => {
...
async fn analyze_with_qwen(snapshot: &Value) -> Result<ModelReport, String> {
  ...
  let text = v.get("choices").and_then(|c| c.as_array()).and_then(|a| a.get(0))
    .and_then(|x| x.get("message")).and_then(|m| m.get("content")).and_then(|t| t.as_str())
    .ok_or_else(|| "Unexpected Qwen response format".to_string())?;

  match serde_json::from_str::<ModelReport>(text) {
    Ok(mut mr) => { mr.provider = "qwen".to_string(); Ok(mr) },
    Err(e) => {
...
pub async fn run_consensus(snapshot: Value, grounded: bool) -> Result<ThreatReport, String> {
  ...
  let g_available = match oxide_core::google_auth::get_access_token().await { Ok(Some(_)) => true, _ => false };
  if g_available { providers.push("gemini"); }
  ...
  let g_fut = if g_available { Some(analyze_with_gemini(&snapshot, true)) } else { None };
  let q_fut = if q_available { Some(analyze_with_qwen(&snapshot)) } else { None };
  let o_fut = if o_available { Some(analyze_with_openai(&snapshot)) } else { None };

  let (g_res, q_res, o_res) = tokio::join!(
    async { if let Some(f) = g_fut { f.await } else { Err("gemini_unavailable".into()) } },
    async { if let Some(f) = q_fut { f.await } else { Err("qwen_unavailable".into()) } },
    async { if let Some(f) = o_fut { f.await } else { Err("openai_unavailable".into()) } },
  );

  let mut reports: Vec<ModelReport> = vec![];
  if let Ok(r) = g_res { reports.push(r); }
  if let Ok(r) = q_res { reports.push(r); }
  if let Ok(r) = o_res { reports.push(r); }

  // Log per-provider confidence and score for debugging
  for r in &reports {
    info!("Provider report: {} -> risk_score={:.1}, confidence={:.2}", r.provider, r.risk_score, r.confidence);
  }
  let elapsed_ms = t0.elapsed().as_millis();
  info!("Consensus completed in {} ms ({} reports)", elapsed_ms, reports.len());
...
mod tests {
  ...
  fn aggregate_empty_reports() {
    ...
    assert!(rep.recommendations.iter().any(|r| r.contains("unable to analyze")));
  ...
  fn aggregate_single() {
    ...
    let r1 = ModelReport {
      ...
      risk_score: 80.0,
      confidence: 0.9,
      findings: vec![ThreatFinding { id: "p1".into(), kind: "process".into(), severity: "high".into(), summary: "sus".into(), rationale: None, indicators: vec!["pid:1".into()] }],
      indicators: vec![Indicator { kind: "proc".into(), value: "pid:1".into(), context: None }],
  ...
  fn aggregate_dual_weighted() {
    ...
    let r1 = ModelReport {
      provider: "gemini".into(), risk_score: 90.0, confidence: 1.0,
      findings: vec![], indicators: vec![], recommendations: vec!["A".into()], citations: vec![]
    };
    let r2 = ModelReport {
      provider: "qwen".into(), risk_score: 10.0, confidence: 0.1,
      findings: vec![], indicators: vec![], recommendations: vec!["B".into()], citations: vec![]
    };
