...
use crate::gemini_api::{Content, Part, FunctionCall, FunctionResponse};
...
impl CopilotAgent {
    pub fn new(config: CopilotConfig, ai_orchestrator: Arc<AIOrchestrator>, function_registry: Arc<FunctionRegistry>) -> Self {
        Self {
    ...
    pub async fn analyze_screen(&self) -> Result<ImageBuffer<Rgba<u8>, Vec<u8>>, CopilotError> {
        ...
        let screenshot = self.screen_capture.capture_screen().await.map_err(|e| CopilotError::ScreenCapture(e))?;
    ...
    pub async fn handle_user_input(&self, user_input: String, context: Context) -> Result<String, CopilotError> {
        ...
        loop {
            ...
            let agent_response_str = self.ai_orchestrator.generate_response(
            ...
            if agent_response_str.starts_with("FUNCTION_CALL:") {
                let function_call_str = agent_response_str.trim_start_matches("FUNCTION_CALL:").trim();
                match serde_json::from_str::<FunctionCall>(function_call_str) {
                    Ok(function_call) => {
                        ...
                        let function_result = self.function_registry.execute_function(&function_call.name, function_call.args.clone()).await.map_err(|e| CopilotError::FunctionExecution(e))?;
                        info!("Function {} executed with result: {}", function_call.name, function_result);
                        ...
                        current_history.push(Interaction {
                            ...
                            agent_response: format!("FUNCTION_CALL: {}", serde_json::to_string(&function_call).map_err(|e| CopilotError::Serialization(e))?),
                            context: context.clone(),
                        });
                        current_history.push(Interaction {
                            id: uuid::Uuid::new_v4(),
                            ...
                            agent_response: format!("FUNCTION_RESPONSE: {}", serde_json::to_string(&FunctionResponse { name: function_call.name, response: function_result }).map_err(|e| CopilotError::Serialization(e))?),
                            context: context.clone(),
                        });
                    }
                    Err(e) => {
