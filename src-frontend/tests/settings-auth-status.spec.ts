...
test.describe('Settings: provider statuses and OpenAI clear session (simulated)', () => {
  test('lists all providers with status badges and updates OpenAI status', async ({ page }) => {
    ...
    // Headings exist
    await expect(page.getByRole('heading', { name: /google gemini/i })).toBeVisible();
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('openai_oauth', { detail: { action: 'clear' } }));
    });
