...
test.describe("System Integration Tests", () => {
  ...
  test("should handle provider selection and persistence", async ({ page }) => {
    ...
    if ((await providerSelectors.count()) > 0) {
      ...
      const geminiProvider = page
        .locator(".provider-btn")
        .filter({ hasText: "Google Gemini" });
      const qwenProvider = page
        .locator(".provider-btn")
        .filter({ hasText: "Qwen" });
      const localProvider = page
        .locator(".provider-btn")
  ...
  test("should handle chat interface with different providers", async ({
    page,
  }) => {
    // Navigate to chat
  ...
  test("should display system dashboard correctly", async ({ page }) => {
    ...
    await expect(
      page.locator(".dashboard-container, .system-dashboard"),
    ).toBeVisible();
  ...
  test("should handle analysis panel functionality", async ({ page }) => {
    ...
    await expect(
      page.locator(".system-analysis-panel, .analysis-panel"),
    ).toBeVisible();

    // Look for analysis buttons or controls
    const analysisButtons = page.locator(
  ...
  test("should integrate collaborative system with main interface", async ({
    page,
  }) => {
