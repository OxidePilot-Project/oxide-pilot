...
test.describe("OpenAI API Key UI", () => {
  test("should show API key input and save successfully", async ({ page }) => {
    ...
    await expect(page.getByText(/api key saved|openai is ready/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("should allow clearing API key session", async ({ page }) => {
    ...
    const clearButton = page.getByRole("button", {
      name: /clear|sign out|disconnect/i,
    });
...
test.describe("OpenAI provider routing (web preview mode)", () => {
  test("should show OpenAI provider badge in Chat when provider is openai", async ({
    page,
    context,
  }) => {
    ...
    await context.addInitScript(() => {
      try {
        localStorage.setItem("oxide.provider", "openai");
      } catch {}
    });
    ...
    await expect(
      page.locator(".provider-badge", { hasText: "OpenAI" }),
    ).toBeVisible();
  });

  test("should route messages to OpenAI when selected", async ({
    page,
    context,
  }) => {
    await context.addInitScript(() => {
