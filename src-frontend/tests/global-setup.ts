...
export default async function globalSetup() {
  ...
  if (dockerMode) {
    try {
      ...
      if (!fs.existsSync(composeFile)) {
        console.warn(
          `[global-setup] Compose file not found at ${composeFile}. Falling back to venv sidecar.`,
        );
      } else {
        console.log(
          "[global-setup] Starting Cognee sidecar via Docker Compose...",
        );
        ...
        const out = execSync(
          `docker compose -f "${composeFile}" port cognee-sidecar 8765`,
        )
          .toString()
          .trim();
        let host = "127.0.0.1";
        ...
        if (out) {
          ...
          host =
            parts.slice(0, -1).join(":").replace(/[[\]]/g, "") || "127.0.0.1";
        ...
        console.log(
          `[global-setup] Cognee sidecar (Docker) is ready at ${host}:${port}.`,
        );
        // Marker for teardown
        try {
          fs.writeFileSync(path.join(repoRoot, "cognee-sidecar.docker"), "1");
        } catch {}
        return;
      }
    } catch (e) {
      console.warn(
        `[global-setup] Docker mode failed: ${(e as Error).message}. Falling back to venv sidecar.`,
      );
    }
  }
  ...
  if (!fs.existsSync(scriptPath)) {
    console.warn(
      `[global-setup] Sidecar script not found at ${scriptPath}. Skipping sidecar start.`,
    );
    return;
  }

  console.log(
    `[global-setup] Starting Cognee sidecar on ${SIDECAR_HOST}:${SIDECAR_PORT} ...`,
  );
  const child = spawn(
    "powershell.exe",
    [
  ...
  child.stdout?.on("data", (d: Buffer) =>
    process.stdout.write(`[sidecar] ${d.toString()}`),
  );
  ...
  try {
    ...
    console.warn(
      `[global-setup] Sidecar readiness check failed: ${(err as Error).message}`,
    );
