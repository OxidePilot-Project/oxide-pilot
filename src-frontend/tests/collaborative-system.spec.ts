...
test.describe('Collaborative LLM System', () => {
  ...
  test('should display collaborative tab in navigation', async ({ page }) => {
    ...
    const collaborativeTab = page.locator('button:has-text("🤝 Collaborative")');
  ...
  test('should navigate to collaborative analysis page', async ({ page }) => {
    ...
    await expect(page.locator('p:has-text("Gemini and Qwen working together")')).toBeVisible();
  });

  test('should display input form for collaborative analysis', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("🚀 Run Collaborative Analysis")')).toBeVisible();
  });
  ...
  test('should validate input before analysis', async ({ page }) => {
    ...
    // Add some input
    await page.fill('textarea[placeholder*="Describe the task"]', 'Test analysis request');

    // Button should now be enabled
    await expect(analyzeButton).toBeEnabled();
  });

  test('should show web preview for collaborative analysis', async ({ page }) => {
    // Navigate to collaborative page
    await page.click('button:has-text("🤝 Collaborative")');

    // Fill in the form
    await page.fill('textarea[placeholder*="Describe the task"]', 'Test system analysis');
    ...
    await expect(page.locator('.metric-label:has-text("Confidence:")')).toBeVisible();
    await expect(page.locator('.metric-label:has-text("Consensus:")')).toBeVisible();

    // Check for primary response
    await expect(page.locator('h4:has-text("🎯 Primary Response")')).toBeVisible();
    await expect(page.locator('.provider-badge.gemini')).toBeVisible();

    // Check for specialized analysis
    await expect(page.locator('h4:has-text("🔍 Specialized Analysis")')).toBeVisible();

    // Check for execution plan
    await expect(page.locator('h4:has-text("📋 Execution Plan")')).toBeVisible();
  });
  ...
  test('should display metrics with proper colors', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder*="Describe the task"]', 'Test analysis');
    await page.click('button:has-text("🚀 Run Collaborative Analysis")');
  ...
  test('should handle different task types', async ({ page }) => {
    ...
    for (const taskType of taskTypes) {
      ...
      // Fill in the form
      await page.fill('textarea[placeholder*="Describe the task"]', `Test ${taskType} analysis`);
