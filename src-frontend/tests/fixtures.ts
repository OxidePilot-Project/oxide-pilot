  ...
  page: async ({ page }, use, testInfo) => {
    ...
    try {
      ...
      if (coverage) {
        ...
        await testInfo.attach("coverage-status.txt", {
          body: Buffer.from("coverage: present", "utf-8"),
          contentType: "text/plain",
        });
      } else {
        ...
        const diagFile = path.join(
          diagDir,
          `diagnostic-${process.pid}-${Date.now()}.txt`,
        );
        fs.writeFileSync(diagFile, "coverage: missing");
        await testInfo.attach("coverage-status.txt", {
          body: Buffer.from("coverage: missing", "utf-8"),
          contentType: "text/plain",
        });
      }
    } catch {
