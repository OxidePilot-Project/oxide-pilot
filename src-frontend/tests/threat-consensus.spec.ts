...
test.describe('Threat Consensus System', () => {
  test('should show consensus analysis with multiple providers', async ({ page, context }) => {
    ...
    const consensusSection = page.locator('[data-testid="threat-consensus"], .threat-consensus');
    ...
    if (await consensusSection.isVisible()) {
      ...
      await expect(consensusSection.getByText(/risk.*score|threat.*level/i)).toBeVisible();
  ...
  test('should display consensus mode (dual/triple)', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
        localStorage.setItem('oxide.gemini.configured', 'true');
        localStorage.setItem('oxide.openai.configured', 'true');
  ...
  test('should show per-provider confidence scores', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
        localStorage.setItem('oxide.gemini.configured', 'true');
        localStorage.setItem('oxide.openai.configured', 'true');
    ...
    const providerScores = page.locator('[data-testid="provider-scores"], .provider-scores');

    if (await providerScores.isVisible()) {
      // Should show individual provider confidence
      await expect(providerScores.getByText(/gemini.*\d+%|openai.*\d+%/i)).toBeVisible();
    }
  });

  test('should handle single provider fallback gracefully', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
  ...
  test('should show disagreement alerts when providers differ significantly', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
        localStorage.setItem('oxide.gemini.configured', 'true');
        localStorage.setItem('oxide.openai.configured', 'true');
    ...
    const disagreementAlert = page.getByText(/disagreement|review.*manually|high diversity/i);
  ...
  test('should display threat findings from consensus', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
        localStorage.setItem('oxide.openai.configured', 'true');
      } catch {}
    ...
    const findingsSection = page.locator('[data-testid="threat-findings"], .threat-findings');

    if (await findingsSection.isVisible()) {
      // Should show finding categories
      await expect(findingsSection.getByText(/process|file|network|config/i)).toBeVisible();

      // Should show severity levels
      await expect(findingsSection.getByText(/low|medium|high|critical/i)).toBeVisible();
    }
  });

  test('should show recommendations based on consensus', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
        localStorage.setItem('oxide.openai.configured', 'true');
      } catch {}
    ...
    if (await recommendations.isVisible()) {
      ...
      await expect(recommendations.locator('li, .recommendation-item')).toHaveCount({ min: 1 });
    }
  });

  test('should update consensus when running new analysis', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
        localStorage.setItem('oxide.openai.configured', 'true');
      } catch {}
    ...
    const analyzeButton = page.getByRole('button', { name: /analyze|scan|run.*consensus/i });
    ...
    if (await analyzeButton.isVisible()) {
      ...
      await expect(page.getByText(/risk.*score|threat.*level/i)).toBeVisible({ timeout: 10000 });
    }
  });
});

test.describe('Threat Consensus Performance', () => {
  test('should complete consensus analysis within reasonable time', async ({ page, context }) => {
    await context.addInitScript(() => {
      try {
        localStorage.setItem('oxide.openai.configured', 'true');
      } catch {}
    ...
    if (await analyzeButton.isVisible()) {
      ...
      // Wait for results
      await expect(page.getByText(/risk.*score|threat.*level/i)).toBeVisible({ timeout: 30000 });
