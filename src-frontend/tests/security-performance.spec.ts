...
test.describe("Security & Performance panels (browser mode)", () => {
  ...
  test("Security tab renders and actions are disabled", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Security Overview/i }),
    ).toBeVisible({ timeout: 20000 });
    await expect(
      page.getByRole("heading", { name: /Security Center/i }),
    ).toBeVisible({ timeout: 20000 });
    ...
    await expect(
      page.getByRole("button", { name: "Validate Session" }),
    ).toBeDisabled();
    await expect(
      page.getByRole("button", { name: "Check Permission" }),
    ).toBeDisabled();
    ...
    if (process.env.PW_VRT === "1") {
      await expect(page).toHaveScreenshot("security-tab.png", {
        fullPage: true,
  ...
  test("Performance tab renders and monitoring/clear actions are disabled", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Performance Alerts & Errors/i }),
    ).toBeVisible({ timeout: 20000 });
    ...
    await expect(
      page.getByRole("heading", { name: "Recent Errors" }),
    ).toBeVisible({ timeout: 20000 });
    await expect(
      page.getByRole("heading", { name: "Operation Profiles" }),
    ).toBeVisible({ timeout: 20000 });
