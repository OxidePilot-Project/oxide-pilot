...
const status = writable<{ message: string; type: StatusType }>({
  message: "",
  type: "info",
});
...
onMount(() => {
  ...
  if (!isTauri && typeof window !== "undefined") {
    const handler = (ev: Event) => {
      try {
        ...
        if (action === "start") {
          ...
          verification_uri =
            detail.verification_uri || "https://example.com/verify";
          expires_in =
            typeof detail.expires_in === "number" ? detail.expires_in : 300;
          intervalSec =
            typeof detail.interval === "number" ? detail.interval : 5;
          if (expires_in) startCountdown(expires_in);
          status.set({
            message:
              "Open the verification URL and enter the code to authorize.",
            type: "info",
          });
          ...
          status.set({
            message: "Authenticated successfully. Tokens stored securely.",
            type: "success",
          });
          isBusy = false;
    ...
    return () =>
      window.removeEventListener("qwen_device_flow", handler as EventListener);
  }
});
...
async function startQwenAuth() {
  if (!isTauri) {
    status.set({
      message: "Not running in Tauri context. Please use the desktop app.",
      type: "error",
    });
    return;
  }
  ...
  try {
    ...
    status.set({
      message: "Open the verification URL and enter the code to authorize.",
      type: "info",
    });
    ...
    status.set({
      message: `Failed to start device auth: ${err}`,
      type: "error",
    });
    isBusy = false;
  }
...
async function pollUntilDone() {
  ...
  try {
    const res = await tauriInvoke<{ status: string; message?: string }>(
...
async function clearQwenSession() {
  if (!isTauri) {
    status.set({
      message: "Clear session is a desktop-only operation in this demo mode.",
      type: "info",
    });
    return;
  }
